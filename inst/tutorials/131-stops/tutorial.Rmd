...
x <- stops |>
  ...
  mutate(
    ...
    arrested = factor(arrested, levels = c(0, 1), labels = c("No", "Yes"))
  )
...
fit_stops <- logistic_reg() |>
    set_engine("glm") |>
    fit(as.factor(arrested) ~ sex + race*zone, data = x)
...
<!-- DK: Change to a logistic regression. Which means you should change the code which creates x so that arrested becomes a factor variable with Yes/No values. DONE-->

<!-- DK Have students run these:
...
```{r the-question-7}
question_text(NULL,
	message = "For each person, there are only two possible potential outcomes for arrested: Yes and No. So, there might be a person who, if she had been wearing a mask, would have been arrested, but, since she was not wearing a mask, she was not arrested. For a given person, the value of the treatment variable might be is wearing a mask or isnt wearing a mask. The causal effect is the comparison between these two potential outcomes — it is not simply a numeric subtraction, but rather the contrast between ‘arrested’ and ‘not arrested’ under the two treatment conditions.",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 6)
...
<!-- DK: Discuss how the causal effect is not a number, so you can't use "subtraction" when it says "difference." Review the relevant portion of the template_tutorial. Done-->
...
```{r wisdom-7}
question_text(NULL,
	message = "We obviously need `race`. However, other characteristics like the person's age, sex, and the type of car they're driving might also affect `race`. It would also be beneficial to see if the zone of the officer impacts the arrest rates, so we should also use the variable `zone`.",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 6)
...
<!-- DK: Make this, and later questions, match the template. -->
...
Continue the pipe to `fit(as.factor(arrested) ~ sex, data = x)`.
...
logistic_reg(engine = "glm") |> 
	fit(as.factor(arrested) ~ sex, data = x)
```
...
Change the call to `fit()` to `fit(as.factor(arrested) ~ race, data = x)`.
...
logistic_reg(engine = ...) |> 
	fit(as.factor(arrested) ~ ..., data = x)
```

```{r courage-9-test, include = FALSE}
logistic_reg(engine = "glm") |> 
	fit(as.factor(arrested) ~ race, data = x)
```
...
Change the call to `fit()` to `fit(as.factor(arrested) ~ sex + race, data = x)`.
...
logistic_reg(engine = "glm") |> 
	fit(as.factor(arrested) ~ race*zone, data = x) |>
  tidy(conf.int = TRUE)
```

### 

<!-- DK: Do not copy/paste. Calculate on the fly. -->

```{r}
logistic_reg(engine = "glm") |> 
	fit(as.factor(arrested) ~ race*zone, data = x) |>
  tidy(conf.int = TRUE) |>  slice_head(n = 5)
```
...
> [ Include what we suggested at the end of Justice.]
...
Recall some of the values for the parameters in our data generating mechanism:
Note that we have only shown the first 5 rows
...
<!-- DK: Print out all the rows. Probably not. Print out the top 5 rows and then note you have cut it off. done-->
...
###
 ...
 Dummy variables must always be interpreted in the context of the base value for that variable, which is generally included in the intercept. For example, the base value here is "black female" (The base value is the first alphabetically by default for character variables. However, if it is a factor variable, you can change that by setting the order of the levels by hand.)
...
### 
 ...
 Group the prediction output by the "sex" variable — but plot them on the same plot, with "sex" on the x-axis or shown as different points (like a bar plot or dot plot).

The x-axis will usually be "sex"

Each "sex" category will have a point + confidence interval for the predicted arrest probability.
...
Condition on different values of "sex" and hold all other variables constant (usually at their reference or average level).

If you do condition = c("sex", "race"), it would show all combinations.

If just condition = "sex", it will show two groups: Male vs. Female, but also controlling for other variables in the model.

It still returns a tibble of predicted probabilities — but the conditioning changes how predictions are generated: holding other variables constant.
